...
preg_match('/ovosh-([0-9]+)/', $_SERVER["REDIRECT_URL"], $id);
$ID_ELEMENT = $id[1];

if($ID_ELEMENT){
	$filter = "id = ".$ID_ELEMENT;
	$arResult = $component->getItems($filter,false,'ovoshchi',false)[0];

	if($arResult["ID_CATEGORY"]){

		$arResult["DOP"] = $component->getItems("","ID,NAME,IMG",'ovoshchi',array("LIMIT"=>4),"order by rand()");
