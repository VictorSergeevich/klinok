...
preg_match('/recipe-([0-9]+)/', $_SERVER["REDIRECT_URL"], $id);
$ID_ELEMENT = $id[1];

if($ID_ELEMENT){
	$filter = "id = ".$ID_ELEMENT;
	$arResult = $component->getItems($filter,false,'elements',false)[0];

	if($arResult["ID_CATEGORY"]){

		$arResult["DOP"] = $component->getItems("id_category = ".$arResult["ID_CATEGORY"],"ID,NAME,IMG",'elements',array("LIMIT"=>4),"order by rand()");
